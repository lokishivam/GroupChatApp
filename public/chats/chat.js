const socket = io("ws://13.210.156.34:3000");
...
async function getOlderMessages() {
  try {
    const response = await axios.get(
      `http://13.210.156.34:3000/messages/get-recent-messages/?oldestMessageLoaded=${currentOpenGroupOldestIndex}&groupId=${currentOpenGroupId}`,
      {
        headers: { token: token },
      }
    );
...
createGroupBtn.addEventListener("click", async () => {
  try {
    ...
    const response = await axios.post(
      "http://13.210.156.34:3000/groups/create-group",
      obj,
      {
        headers: { token: token },
      }
...
async function getAllGroups() {
  try {
    const response = await axios.get(
      "http://13.210.156.34:3000/groups/get-all-groups",
      {
        headers: { token: token },
      }
    );
...
addUserModalFormBtn.addEventListener("click", async () => {
  try {
    ...
    const result = await axios.post(
      "http://13.210.156.34:3000/admin/add-to-group",
...
removeUserModalFormBtn.addEventListener("click", async () => {
  try {
    ...
    console.log(currentOpenGroupId);
    const result = await axios.post(
      "http://13.210.156.34:3000/admin/remove-from-group",
      { userEmail, groupId: currentOpenGroupId },
      { headers: { token } }
    );
    adminResult.classList.add("bg-info");
...
makeAdminModalFormBtn.addEventListener("click", async () => {
  try {
    ...
    console.log(currentOpenGroupId);
    const result = await axios.post(
      "http://13.210.156.34:3000/admin/make-user-admin",
      { userEmail, groupId: currentOpenGroupId },
      { headers: { token } }
    );
    adminResult.classList.add("bg-info");
